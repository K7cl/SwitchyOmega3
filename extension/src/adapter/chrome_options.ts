...
// docs/decisions.md. Per-tab dynamic icons are deferred; a global action
...
export class ChromeOptions extends Options {
  ...
  override currentProfileChanged(reason?: unknown): void {
    ...
    chrome.action.setTitle({ title: title || 'Proxy SwitchyOmega' })

    let icon
    if (!real || !real.name || !Profiles.isInclusive(real)) {
      icon = drawIcon(real?.color ?? '#49afcd')
    } else {
      icon = drawIcon(real.color!, this.profile('direct')?.color)
    }
